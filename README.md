...
By default, when a `_resolve_reference` field returns a struct, the library resolves the `_Entity` union by matching the last segment of the struct's module name against your schema's object type names. If your struct's module name does not match a type name in your schema, this match fails.
