defmodule Absinthe.Federation.Schema.Phase.Validation.KeyFieldsMustBeValidWhenExtends do
  ...
  """
  def run(bp, _) do
    bp = Blueprint.prewalk(bp, &handle_schemas(&1, bp.adapter))
    {:ok, bp}
  end
  ...
  defp validate_object(obj, _adapter), do: obj
  ...
  defp is_marked_as_external?(key, fields, adapter) do
    adapter = adapter || Absinthe.Adapter.LanguageConventions
    internal_name = adapter.to_internal_name(key, :field)
    IO.inspect(key, label: "key")
    IO.inspect(internal_name, label: "internal_name")
    IO.inspect(fields, label: "fields")
    field = Enum.find(fields, &(internal_name == &1.name))
    field && field.directives != [] && is_external?(field)
  end

  defp is_external?(field) when is_struct(field) do
    Enum.find(field.directives, fn x -> x.name == "external" end)
  end

  defp is_external?(_field), do: false
